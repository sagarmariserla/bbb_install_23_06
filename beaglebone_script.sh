...
sudo chmod +x firmware_update_arm frmwre_update_failed_arm frmwre_update_success_arm script_failed_arm script_success_arm software_update.sh cron_job checking_firmware hard_reset_arm
sudo chmod +x $(pwd)/config_change/config

sudo mv -f $(pwd)/iot_frm* zwave_app* app.cfg cmd_class.cfg zwave_device_rec.txt hard_reset_arm cron_job firmware_update_arm frmwre_update_failed_arm frmwre_update_success_arm script_failed_arm script_success_arm checking_firmware status_firmware.txt software_update.sh version config_change/ /medha_gateway
...
sudo dpkg -i $(pwd)/Z_wave_intall_beaglebone/zipgateway-7.15.04-Linux-stretch-armhf.deb 
sudo apt-get --fix-broken install -y

#installing the zware
echo "*******************************************************************************unpacking the zwave tar file*********************************************************************"

tar -xf $(pwd)/Z_wave_intall_beaglebone/zware-7.15.4-rpi.tar.gz -C $(pwd)/Z_wave_intall_beaglebone/

echo "*******************************************************************************installing the  zware****************************************************************************"
bash $(pwd)/Z_wave_intall_beaglebone/zware-7.15.4-rpi/install.sh  /medha_gateway/local_server
...
echo "******************************************************************************booting script**************************************************************************************"
sudo bash $(pwd)/booting_script.sh iot_frmwrk
sudo bash $(pwd)/booting_script.sh zwave_app
sudo bash $(pwd)/booting_script.sh firmware_update_arm
sudo bash $(pwd)/booting_script.sh checking_firmware
